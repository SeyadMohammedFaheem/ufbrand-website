...
export function ProductInfo({ product }) {
  ...
  const whatsappUrl = (() => {
    ...
    const phone = process.env.NEXT_PUBLIC_WHATSAPP_NUMBER || '916379439162';
  ...
  return (
    <div className="flex flex-col w-full">
      ...
      <div className="flex items-center gap-1.5 mb-7">
        ...
        <span className="text-[11px] text-zinc-400 font-medium">Ships in 2–4 business days</span>
      </div>
          ...
          <div className="flex flex-wrap gap-2">
            {product.sizes.map((size) => (
              <button
                ...
                className={`min-w-[48px] h-11 px-4 flex items-center justify-center border text-[12px] font-black tracking-wider transition-all rounded-lg ${
                  selectedSize === size
      ...
      <div ref={actionRef} className="flex flex-col gap-2.5 mb-8">
        <button
          ...
          className={`w-full py-4 font-black uppercase tracking-widest text-sm transition-all flex items-center justify-center gap-2 rounded-xl ${
            added
      ...
      {/* Trust Badges */}
      <div className="grid grid-cols-3 gap-3 bg-[#FAF9F6] rounded-xl p-4 mb-8">
        <div className="flex flex-col items-center justify-center text-center gap-1.5">
          <Truck size={20} strokeWidth={1.5} className="text-[#30323E]" />
          <span className="text-[9px] font-bold uppercase tracking-wider text-zinc-600 leading-tight">Pan-India<br/>Shipping</span>
        </div>
        <div className="flex flex-col items-center justify-center text-center gap-1.5 border-x border-zinc-200">
        ...
        <Accordion title="Shipping & Delivery">
          <div className="space-y-3 text-sm text-zinc-600">
            <div className="flex items-start gap-2">
              <Truck size={14} className="text-[#D4147A] mt-0.5 flex-shrink-0" />
              <p>Pan-India shipping available. Delivered in <strong>2–4 business days</strong>.</p>
            </div>
            <div className="flex items-start gap-2">
      ...
      <div className="mt-6 p-4 bg-zinc-50 rounded-xl border border-zinc-100 mb-20 md:mb-0">
        <p className="text-[10px] text-zinc-400 font-medium leading-relaxed">
          ...
          Have questions? WhatsApp us at <a href="https://wa.me/919899990772" className="text-[#D4147A] font-bold">+91 9899990772</a>.
        </p>
      </div>
      ...
      >
        <button
          onClick={handleAddToCart}
          className={`flex-1 py-3.5 font-black uppercase tracking-widest text-[11px] transition-all flex items-center justify-center gap-2 rounded-xl ${
            added
              ? 'bg-green-500 text-white'
